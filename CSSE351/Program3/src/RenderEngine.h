...
public:
    ...
    void display(WorldState const & world)
    {
        ...
        // DONE: use a transform to rotate the large hand clockwise
        // DONE: use a transform to rotate the small hand clockwise at 1/12 the large hand rate
        
        //TODO handle the qwer transforms
        //apply the z rotation before the y rotation
        //figure out the rest of the transform order
        
        glm::mat4 worldState = glm::mat4(1.0f);
        
        if(world.scaleEnabled()) {
            worldState = glm::scale(glm::mat4(1.0f), glm::vec3(0.4, 0.4, 0.4)) * worldState;
        }
        if(world.rotYEnabled()) {
            worldState = glm::rotate(glm::mat4(1.0f), time, glm::vec3(0, 1, 0)) * worldState;
        }
        if(world.rotZEnabled()) {
            worldState = glm::rotate(glm::mat4(1.0f), time, glm::vec3(0, 0, 1)) * worldState;
        }
        if(world.translateEnabled()) {
            worldState = glm::translate(glm::mat4(1.0f), glm::vec3(world.getXPos(), world.getYPos(), 0)) * worldState;
        }
        ...
        // ticks
        for(float i = 0; i < M_PI * 2; i += M_PI * 2 / 12) {
            glm::mat4 tickRot = glm::rotate(glm::mat4(1.0f), i, glm::vec3(0, 0, 1));
            glm::mat4 tickTrans = glm::translate(glm::mat4(1.0f), glm::vec3(0.6 * cos(i), 0.6 * sin(i), 0));
            glm::mat4 tickComp = worldState * tickTrans * tickRot;
            glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &tickComp[0][0]);
            glDrawArrays(GL_LINES, model.getObjectStart(1), model.getObjectSize(1));
        }
        
        // small hand
        glm::mat4 small = worldState * glm::rotate(glm::mat4(1.0f), time/12, glm::vec3(0, 0, -1));
        glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &small[0][0]);
        glDrawArrays(GL_LINES, model.getObjectStart(2), model.getObjectSize(2));
        
        // large hand
        glm::mat4 large = worldState * glm::rotate(glm::mat4(1.0f), time, glm::vec3(0, 0, -1));
        glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &large[0][0]);
        glDrawArrays(GL_LINES, model.getObjectStart(3), model.getObjectSize(3));
        
        // outline
        glm::mat4 outline = worldState * glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, 0));
        glUniformMatrix4fv(transformSlot, 1, GL_FALSE, &outline[0][0]);
        glDrawArrays(GL_LINE_LOOP, model.getObjectStart(4), model.getObjectSize(4));
