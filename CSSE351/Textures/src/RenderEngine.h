...
public:
	...
	void display(WorldState & state)
	{
		...
		for(int i=0; i<numQuads; i++)
		{
            ...
            // texture first unit
            GLint texUnitId = 0;
            gl3wActiveTexture(GL_TEXTURE0 + texUnitId);
...
private:
	...
	void setupTextures()
	{
		...
		for(int i=0; i<4; i++)
		{
            ...
            gl3wBindTexture(GL_TEXTURE_2D, textures[i]);

			//TODO set min/mag sampling parameters
            gl3wTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            gl3wTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
            ...
            gl3wBindTexture(GL_TEXTURE_2D, 0);
		}
